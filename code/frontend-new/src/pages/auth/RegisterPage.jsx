...
export default function RegisterPage() {
  ...
  const [regProbeId, setRegProbeId] = useState('');
  ...
  const handleRegisterSubmit = async (e) => {
    ...
    try {
      await register({
        ...
        reason: regReason,
        probeId: regRole === 'RESIDENT' ? regProbeId : null,
      });
        ...
        <div className="login-page__content-centered">
          <Card className="login-role-card" style={{ width: '100%', maxWidth: '480px' }}>
            ...
            <form onSubmit={handleRegisterSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              ...
              {regRole === 'RESIDENT' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', textAlign: 'left' }}>
                  <label htmlFor="regProbeId">Probe ID</label>
                  <input 
                    id="regProbeId" 
                    type="text" 
                    value={regProbeId} 
                    onChange={(e) => setRegProbeId(e.target.value)} 
                    required={regRole === 'RESIDENT'} 
                    placeholder="Enter associated Probe ID"
                    style={{ padding: '0.75rem', borderRadius: '4px', border: '1px solid #ccc' }} 
                  />
                </div>
              )}
